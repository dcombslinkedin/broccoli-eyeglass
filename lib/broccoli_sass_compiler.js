...
function defaultOptionsGenerator(sassFile, cssFile, options, cb) {
  let h = heimdall.start("broccoliSassCompiler:defaultOptionsGenerator");
...
function parsePath(pathname) {
  let h = heimdall.start("broccoliSassCompiler:parsePath");
...
function formatPath(parsed) {
  let h = heimdall.start("broccoliSassCompiler:formatPath");
...
function writeDataToFile(cachedFile, outputFile, data) {
  let h = heimdall.start("broccoliSassCompiler:writeDataToFile");
...
module.exports = class BroccoliSassCompiler extends BroccoliPlugin {
  ...
  compileSassFile(srcPath, sassFilename, destDir) {
    let h = heimdall.start("broccoliSassCompiler:compileSassFile");
  ...
  dependencyChanged(srcDir, dep) {
    let h = heimdall.start("broccoliSassCompiler:dependencyChanged");
  ...
  getFromCacheOrCompile(details) {
    let h = heimdall.start("broccoliSassCompiler:getFromCacheOrCompile");
  ...
  hashForFile(absolutePath) {
    let h = heimdall.start("broccoliSassCompiler:hashForFile");
  ...
  keyForSourceFile(srcDir, relativeFilename, options) {
    let h = heimdall.start("broccoliSassCompiler:keyForSourceFile");
  ...
  handleCacheHit(details, inputAndOutputFiles) {
    let h = heimdall.start("broccoliSassCompiler:handleCacheHit");
  ...
  hashDependencies(details) {
    let h = heimdall.start("broccoliSassCompiler:hashDependencies");
   ...
   */
  readOutputs(details) {
    let h = heimdall.start("broccoliSassCompiler:readOutputs");
  ...
  populateCache(key, details, result) {
    let h = heimdall.start("broccoliSassCompiler:populateCache");
    ...
    try {
      ...
      let h1 = heimdall.start('broccoliSassCompiler:populateCache.stringifyDeps');
      let depsWithHashesStr = JSON.stringify(depsWithHashes);
      h1.stop();;

      let h2 = heimdall.start('broccoliSassCompiler:populateCache.stringifyOutput');
      let outputContentsStr = JSON.stringify(outputContents);
      h2.stop();

      cache.set(this.dependenciesKey(key), depsWithHashesStr);
      cache.set(this.outputKey(key), outputContentsStr);
    } finally {
      h.stop();
    }
  }
  ...
  compileCssFile(details) {
    let h = heimdall.start("broccoliSassCompiler:compileCssFile");
    
    try {
      this.compileCssFileStart = Date.now();
//      console.log("### Start Compiling CSS file (in BSC) at " + this.compileCssFileStart);
...
//        console.log("### After 'compiling' event emitted, at " + Date.now());
      
        let dependencyListener = absolutePath => {
//          console.log("### BSC dependencyListener add for fullSassFilename " + details.fullSassFileName +
//                      " at abs path " + absolutePath + ", at " + Date.now());
          this.addDependency(details.fullSassFilename, absolutePath);
        };

        let additionalOutputListener = filename => {
//          console.log("### BSC additionalOutputListener add for fullSassFilename " + details.fullSassFileName +
//                      " at filename " + filename + ", at " + Date.now());
        ...
        return Promise.resolve(sass(details.options))
          .finally(() => {
//            console.log("### After sass call resolves promise, at " + Date.now());
          ...
          .then(result => {
//            console.log("### Just before call success, at " + Date.now());
  ...
  handleSuccess(details, result) {
    let compileStart = this.compileCssFileStart;
    let compileEnd = Date.now();

    let h = heimdall.start("broccoliSassCompiler:handleSuccess");
    
    try {
//      console.log("Handle Success of compiling CSS file (in BSC) at " + compileEnd + " (total time " + (compileEnd - compileStart) + " ms)");
  ...
  filesInTree(srcPath) {
    let h = heimdall.start("broccoliSassCompiler:filesInTree");
  ...
  _build() {
    let h = heimdall.start("broccoliSassCompiler:_build");
